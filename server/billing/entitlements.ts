...
const PLAN_FEATURES: Record<Plan, Record<FeatureKey, boolean>> = {
  free: {
...
const PLAN_LIMITS: Record<Plan, Record<UsageMetric, number | null>> = {
  free: {
    "connectors.active": 2,
...
function hasInternalBypass(role: AxiomRole): boolean {
  return role === "admin" || role === "service";
}

function cloneFeatures(plan: Plan): Record<FeatureKey, boolean> {
  return { ...PLAN_FEATURES[plan] };
}

function cloneLimits(plan: Plan): Record<UsageMetric, number | null> {
  return { ...PLAN_LIMITS[plan] };
}

function normalizePlan(state: BillingState, now: Date): {
  plan: Plan;
...
export function resolveEntitlements(input: {
  ...
  for (const [feature, enabled] of Object.entries(input.state.overrides)) {
    ...
    features[feature as FeatureKey] = enabled;
  }
