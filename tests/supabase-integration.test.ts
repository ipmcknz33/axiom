...
const haveCreds = Boolean(SUPABASE_URL && SERVICE_ROLE_KEY);

test("integration: supabase setup", async (t) => {
  if (!RUN_LIVE_TESTS) {
    t.skip("RUN_LIVE_SUPABASE_TESTS not set");
    return;
  }
  if (!haveCreds) {
    console.log(
...
test("persistence: insert approval record to database", async (t) => {
  if (!RUN_LIVE_TESTS || !haveCreds) {
    t.skip("live supabase tests disabled");
    return;
  }
...
test("persistence: list approval records from database", async (t) => {
  if (!RUN_LIVE_TESTS || !haveCreds) {
    t.skip("live supabase tests disabled");
    return;
  }
...
test("persistence: insert audit log record to database", async (t) => {
  if (!RUN_LIVE_TESTS || !haveCreds) {
    t.skip("live supabase tests disabled");
    return;
  }
...
test("health: database connectivity check via profiles query", async (t) => {
  if (!RUN_LIVE_TESTS || !haveCreds) {
    t.skip("live supabase tests disabled");
    return;
  }
